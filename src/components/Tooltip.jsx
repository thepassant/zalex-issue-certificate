...
const Tooltip = ({
  ...
  const getStylesList = useCallback(() => {
    if (tooltipWrapperRef.current) {
      const wrapperRect = tooltipWrapperRef.current.getBoundingClientRect(),
        ...
        centeredHorizontalPosition = Math.max(
          space,
      ...
      //if position is right and no room for tooltip => change position to left
        ...
        wrapperRect.right + ((childrenWidth || 0) + space * 1.5) >
          window.innerWidth
      ) {
      ...
      if (pos === availableTooltipPositions.top) {
        ...
        style.left = Math.max(
          space,
          wrapperRect.left - ((childrenWidth || 0) + space)
        );
      }
  ...
  useEffect(() => {
    ...
    if (
      getStylesList().top !== styles.top ||
      getStylesList().left !== styles.left
    ) {
      setStyles(getStylesList());
    }
  }, [getStylesList, styles.left, styles.top]);

  const updateScrollableParentScroll = ({
  ...
  useEffect(() => {
    ...
    if (tooltipWrapperRef.current) {
      const wrapperRef = tooltipWrapperRef.current,
        scrollableParent = getScrollParent(wrapperRef),
        newScrollableParent =
          scrollableParent === document.body ? window : scrollableParent;
  ...
  return (
    <ConditionalWrapper
      ...
      wrapper={(children) => (
        <ClickAwayWrapper onClickAwayCallback={hideTooltip}>
          {children}
        </ClickAwayWrapper>
      )}
    >
      <span
        ...
        onClick={
          isClickTrigger ? (disabled ? undefined : showTooltip) : undefined
        }
